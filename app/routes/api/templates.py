...
def list_templates():
    ...
    if db is None:
        return jsonify({"error": "Database connection failed or configuration is invalid"}), 500
...
def create_template():
    ...
    if db is None:
        return jsonify({"error": "Database connection failed. Check server logs and configuration."}), 500
